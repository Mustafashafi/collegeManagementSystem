...
router.post('/send', async (req, res) => {
  ...
  if (!process.env.RESEND_API_KEY || !process.env.EMAIL_FROM) {
    return res.status(500).json({
      success: false,
      msg: 'Resend is not configured properly.'
    });
  }
  ...
  try {
    for (const { email, name } of recipients) {
      await resend.emails.send({
        from: `EduSystem Admissions <${process.env.EMAIL_FROM}>`,
        to: email,
        subject: subject,
        html: `
          <div style="font-family: Inter, sans-serif; max-width: 600px; margin: 0 auto; padding: 30px;">
            ...
            <div style="background: #fff; border: 1px solid #e5e7eb; border-top: none; padding: 30px; border-radius: 0 0 12px 12px;">
              ...
              <p style="font-size: 12px; color: #9ca3af;">
                This email was sent by EduSystem Admissions.
              </p>
            </div>
          </div>
        `
    ...
    return res.json({
      success: true,
      msg: `Successfully sent ${recipients.length} emails.`,
